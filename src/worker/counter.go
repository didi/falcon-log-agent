...
// 只做更新和删除，添加 由数据驱动
func (this *GlobalCounter) UpdateByStrategy(globalStras map[int64]*scheme.Strategy) {
...
func (this *GlobalCounter) AddStrategyCount(st *scheme.Strategy) {
	this.Lock()
...
func (this *GlobalCounter) upStrategy(st *scheme.Strategy) {
	this.Lock()
	if _, ok := this.StrategyCounts[st.ID]; ok {
